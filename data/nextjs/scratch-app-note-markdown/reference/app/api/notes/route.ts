...
export async function GET(req: Request): Promise<Response> {
  ...
  const base = listNotes({ tag: params.get('tag'), q: params.get('q') })
  const notes = params.get('render') === 'html' ? base.map(renderNote) : base
  return json({ notes })
}
...
export async function POST(req: Request): Promise<Response> {
  ...
  const note = createNote({ title: title.trim(), body: noteBody, tags })
