...
describe('navigation', () => {
  ...
  it('navigates to budgets and back to overview', async () => {
    const user = userEvent.setup()
    render(<App />)
    await user.click(screen.getByTestId('nav-budgets'))
    expect(screen.getByTestId('page-budgets')).toBeInTheDocument()
    await user.click(screen.getByTestId('nav-overview'))
    expect(screen.getByTestId('page-overview')).toBeInTheDocument()
    expect(screen.getByTestId('nav-budgets')).not.toHaveAttribute('aria-current')
  })
})
