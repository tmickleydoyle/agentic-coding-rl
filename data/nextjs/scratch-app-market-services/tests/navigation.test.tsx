import { describe, it, expect } from 'vitest'
import { render, screen } from '@testing-library/react'
import userEvent from '@testing-library/user-event'
import App from '../app/page'

describe('navigation', () => {
  it('renders gigs by default with active nav', () => {
    render(<App />)
    expect(screen.getByTestId('page-gigs')).toBeInTheDocument()
    expect(screen.getByTestId('nav-gigs')).toHaveAttribute('aria-current', 'page')
    expect(screen.getByTestId('nav-book')).not.toHaveAttribute('aria-current')
  })

  it('navigates to my bookings', async () => {
    const user = userEvent.setup()
    render(<App />)
    await user.click(screen.getByTestId('nav-bookings'))
    expect(screen.getByTestId('page-bookings')).toBeInTheDocument()
    expect(screen.getByTestId('nav-bookings')).toHaveAttribute('aria-current', 'page')
  })

  it('shows no-selection on detail and book when nothing selected', async () => {
    const user = userEvent.setup()
    render(<App />)
    await user.click(screen.getByTestId('nav-detail'))
    expect(screen.getByTestId('no-selection')).toBeInTheDocument()
    await user.click(screen.getByTestId('nav-book'))
    expect(screen.getByTestId('no-selection')).toBeInTheDocument()
  })

  it('defaults to light theme on the root', () => {
    render(<App />)
    expect(screen.getByTestId('app-root')).toHaveAttribute('data-theme', 'light')
  })
})
