...
export function Roadmap() {
  const { items, addItem, shipItem, quarterFilter, setQuarterFilter } = useApp()
  const [title, setTitle] = useState('')
  const [quarter, setQuarter] = useState<Quarter>('Q1')
  const [status, setStatus] = useState<Status>('planned')

  const visible = quarterFilter === 'All' ? items : items.filter((i) => i.quarter === quarterFilter)
  const shippedCount = visible.filter((i) => i.status === 'shipped').length
